...
function checkForArbs(exchange, data) {
  ...
  if((exchange == 'betfair') && ((data.betType == 'b0') || (data.betType == 'l0'))) {
    if(data.betType == 'b0') {// check if b0
      if((!arbTrigger.matchbook.l0.odds) || (!arbTrigger.matchbook.l0.liquidity)) {// check if matchbook l0 odds not initialized
        ...
        if(((B0O - 1) / (L0O - 1)) > 1.02) {// candidate exists
          ...
          // create shallow copy of betfairDeltas, matchbookDeltas and currentArb
          let
            B = Object.assign({}, betfairDeltas),
            S = Object.assign({}, matchbookDeltas),
            C_Arb = Object.assign({}, currentArb);
          log.info('created shallow copies of betfairDeltas, matchbookDeltas and currentArb...');
          ...
          const arbsDoc = {
            ...
            timestampTo: '',
            summary: `Bet ${SELECTION} on Betfair for £${targetLiquidity} at ${B0O}, Lay on Matchbook for £${targetLiquidity} at ${L0O}. Win: ${WINAMT}. Lose: ${LOSEAMT}, Max: £${maxLiquidity}`,
            b: B,
            s: S
          };
      ...
      if((!arbTrigger.matchbook.b0.odds) || (!arbTrigger.matchbook.b0.liquidity)) {// check if matchbook b0 not initialized
  ...
  else if((exchange == 'matchbook') && ((data.betType == 'b0') || (data.betType == 'l0'))) {
    if(data.betType == 'b0') {// check if b0
      if((!arbTrigger.betfair.l0.odds) || (!arbTrigger.betfair.l0.liquidity)) {// check if betfair l0 not initialized
        ...
        log.info('matchbook b0 seen - no betfair l0');
        return log.info(arbTrigger);
      }
      else {// check if arbs candidate exists
        const
        ...
        if(((B0O - 1) / (L0O - 1)) > 1.02) {// candidate exists
          ...
          const arbsDoc = {
            ...
            timestampFrom: data.timestamp,
            timestampTo: '',
            summary: `Bet ${SELECTION} on Matchbook for £${targetLiquidity} at ${B0O}, Lay on Betfair for £${targetLiquidity} at ${L0O}. Win: ${WINAMT}. Lose: ${LOSEAMT}, Max: £${maxLiquidity}`,
            b: B,
            s: S
          };
      ...
      if((!arbTrigger.betfair.b0.odds) || (!arbTrigger.betfair.b0.liquidity)) {// check if oppossing cell not initialized
        ...
        log.info('matchbook l0 seen - no betfair b0');
        return log.info(arbTrigger);
      }
      else {// check if arbs candidate exists
        const
